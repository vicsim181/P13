...
export default {
  ...
  async fetch() {
    const response_1 = await this.$axios.get('project_type', {
      params: { name: 'Pétition' }
    });
    const petition_type = response_1.data['id_project_type'];
    console.log(petition_type);
    const data = { owner_id: this.user_id, project_type: petition_type };
